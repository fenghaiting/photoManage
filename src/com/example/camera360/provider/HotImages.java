...
public class HotImages extends BaseImages {
	 ...
	 * 表示设置HotImages私有静态对象
	 */
	private static HotImages self = null;

	public HotImages(Fragment fragment) {
		super();
		self = this;
	...
	/**
	 *	@activeName getInstance 表示HotImages实例对象
	 *	@param 无
	 *	@return HotImages
	 */
	public static HotImages getInstance() {
		if(self == null){
			self = new HotImages();
		}
		return self;
	...
	public void doImageUrls() {
		...
		new HotServer(SITE_URL + "activity/picture/hot", paramsList);
	}
	...
	private class HotServer extends Server {

		private Task task = new Task();

		public HotServer(String url, String params) {
			super(params);
			task.execute(url);
		}

		public HotServer(String url, List<NameValuePair> params) {
			super(params);
			task.execute(url);
		}
