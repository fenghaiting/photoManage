...
public class LatestImages extends BaseImages {
	...
	/**
	 * 表示设置LatestImages私有静态对象
	 */
	private static LatestImages self = null;

	public LatestImages(Fragment fragment) {
		super();
		self = this;
	...
	/**
	 *	@activeName getInstance 表示LatestImages实例对象
	 *	@param 无
	 *	@return LatestImages
	 */
	public static LatestImages getInstance() {
		if (self == null) {
			self = new LatestImages();
		}
		return self;
	}
	...
	public void doImageUrls() {
		...
		new LatestServer(SITE_URL + "activity/picture/latest", paramsList);
	}
	...
	private class LatestServer extends Server {
		
		private Task task = new Task();

		public LatestServer(String url, String params) {
			super(params);
			task.execute(url);
		}

		public LatestServer(String url, List<NameValuePair> params) {
			super(params);
			task.execute(url);
		}
		...
		private class Task extends NetAndroidTask {
			...
			protected void onPostExecute(String result) {
				Log.v("result", result);
				LatestImages.getInstance().setImageUrls(result);
			}
		}
	}
}